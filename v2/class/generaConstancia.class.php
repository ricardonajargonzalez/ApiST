...
class generaConstancia {
  ...
  private $message;
      ...
      public function __construct($dataarray){
        ...
        $dir = getcwd() .'/class/'. $this->idarchivador;
        $filecookies = getcwd() .'/class/'. $this->idarchivador. '/cookies.txt';
        //CREACION DEL DIRECTORIO PARA EL GUARDADO DE COOKIES POR ARCHIVADOR
        if(!is_dir($dir)){
           $directoriocookies = mkdir($dir, 0777, true); //se crea directorio
        }
        //CREACION DEL ARCHIVO COOKIES.TXT
        $archivo = fopen($filecookies, "w");
        fclose($archivo);
        $this->cookie = getcwd() .'/class/'.$this->idarchivador.'/cookies.txt';
    ...
    public function login_docuware(){

                  $curl = curl_init();
                  curl_setopt_array($curl, array(
                  CURLOPT_URL => $this->urlnube . '/docuware/platform/Account/Logon',
                  ...
                  CURLOPT_COOKIEJAR => $this->cookie, // se expecifica en donde se guarde la cookie
                  CURLOPT_POSTFIELDS => 'LicenseType=&Password='.$this->passworddw.'&RedirectToMyselfInCaseOfError=false&RememberMe=false&UserName=' . $this->userdw,
                ...
                if($retcode != 200){
                  $this->message = isset($json->Message) ? $json->Message : "Erorr al conectarse con docuware";
                  $this->login   = isset($json->StatusCode) ? $json->StatusCode : $retcode;
                }else{
   ...
   public function base64Tofile($stringbase64,$nombreArchivo){ //constancia(base64), nombre archivo original sin extension
    ...
    public function download_document(){
        ...
        if($getdocument['code']==200){
                $document = json_decode($getdocument['documento']);
                IF($this->allowedResourceTypes($document->ContentType)){
                  ...
                  if ($err) {
                    $error = "true";
                    $message = $this->message;
                    $respuesta = '{"error": '.$error.',"ContentType" : "" ,"message": "Error al descargar documento de Docuware","id_document": "", "hash": ""}';
                    die($respuesta);
                  } else {
                     //DESCARGAMOS EL PDF DE DOCUWARE Y LO GUARDAMOS EN EL SERVIDOR
                      $result_archivo = $this->guardarArchivoServidor($document,$response);
                     if($result_archivo['error'] == false){
                        ...
                        if($result['error'] == false){
                           ...
                           if($result['error'] == false){
                               ...
                               // var_dump("error al subir pdf validador")
                                $error = "true";
                                $message = $this->message;
                                $respuesta = '{"error": '.$error.',"ContentType" : "" ,"message": "Error al subir constancia a Docuware","id_document": "", "hash": ""}';
                                die($respuesta);
                           }
                        }else{
                          // var_dump("error al validar constancia")
                                $error = "true";
                                $message = $this->message;
                                $respuesta = '{"error": '.$error.',"ContentType" : "" ,"message": "Error al validar constancia","id_document": "", "hash": ""}';
                                die($respuesta);
                        }
                     }else{
                       //var_dump(" error en generar constancia");
                        $error = "true";
                        $message = $this->message;
                        $respuesta = '{"error": '.$error.',"ContentType" : "" ,"message": "Error al generar constancia NOM151","id_document": "", "hash": ""}';
                        die($respuesta);
                     }
                    ...
                    //archivo invalido
                     ...
                     $respuesta = '{"error": '.$error.',"ContentType" : "'.$document->ContentType.'" ,"message": "'.$message .'","id_document": "", "hash": ""}';
